...
# Only the Veil of Darkness tab is active right now (see nav_panel section
# below) -- the rest are commented out, not deleted, so uncommenting any of
# these source() lines plus its nav_panel/module_server call brings that
# tab back.
...
ui <- page_fluid(
  ...
  # Sidebar (outcome selector + intersectional-controls checkboxes) is
  # commented out along with every tab except Veil of Darkness below --
  # nothing active reads input$outcome_var / input$controls right now.
  # sidebar = sidebar(
...
server <- function(input, output, session) {
  # Everything below except the Veil of Darkness module call is commented
  # out along with its tab/sidebar controls above -- nothing else is wired
  # up right now. Uncomment alongside the matching nav_panel/source() lines
  # to bring a tab back.
